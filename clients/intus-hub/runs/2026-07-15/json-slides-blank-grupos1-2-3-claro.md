# Blank-style — Slides Internos — Grupos 1, 2 e 3 (versão CLARA/off-white, corrigida)

> Correção sobre a primeira versão: fundo trocado de preto (#0D0D0D) para off-white (#F7F5F1), header trocado para "@diego.spanevello | Inteligência Artificial", footer trocado para texto puro "INTUS HUB" (sem logomark). Fontes mantidas iguais à versão anterior.
> `asset_prompt` continua em PLACEHOLDER — nunca baked-in.

---

## GRUPO 1 — Headline → Imagem full-width → Texto abaixo (CLARA)

```json
{
  "template": "carrossel-blank-style",
  "version": "grupo-1-headline-imagem-texto-clara",
  "format": {
    "aspect_ratio": "4:5",
...
## GRUPO 2 — Imagem lateral (esquerda) + Texto (direita), 2 colunas (CLARA)

```json
{
  "template": "carrossel-blank-style",
  "version": "grupo-2-split-imagem-texto-clara",
  "format": {
    "aspect_ratio": "4:5",
    "resolution": "1080x1350px",
    "background_color": "#F7F5F1"
...
## GRUPO 3 — Texto puro, sem imagem (CLARA)

```json
{
  "template": "carrossel-blank-style",
  "version": "grupo-3-texto-puro-clara",
  "format": {
    "aspect_ratio": "4:5",
    "resolution": "1080x1350px",
    "background_color": "#F7F5F1"
